...
function toggleMenu(nav, navSections, forceExpanded = null) {
  const expanded = forceExpanded !== null
    ? !forceExpanded
    : nav.getAttribute('aria-expanded') === 'true';expandedforceExpandednull!forceExpanded
  const button = nav.querySelector('.nav-hamburger button');
  document.body.style.overflowY = expanded || isDesktop.matches ? '' : 'hidden';expandedisDesktop.matches
...
export default async function decorate(block) {
  ...
  const navPath = navMeta ? new URL(navMeta).pathname : '/nav';navPathnavMetanewURL.pathname
  ...
  if (brandLink) {
    ...
    brandLink.innerHTML = `
      <img src="/logos/${
  isDesktop.matches ? 'logo.svg' : 'logo-mobile.svg'
